...
class QueryParametersInspector(MethodArgumentsInspector):
    ...
    def _query_arguments(self, route) -> typing.List[typing.Tuple[ComponentMethodArgument, str]]:
        ...
        annotations = route.method.annotations.get(MapQueryParameterAnnotation)

        for map_query_parameter_annotation in annotations:
            argument = route.method.get_argument(map_query_parameter_annotation.map_to)
            if argument is not None:
                query_arguments.append((argument, map_query_parameter_annotation.name))
