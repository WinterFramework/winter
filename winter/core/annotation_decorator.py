...
def annotate(annotation: typing.Any, *, key=None, unique=False, single=False)-> typing.Callable:
    def wrapper(
        ...
        if isinstance(func_or_cls, ComponentMethod) or isinstance(func_or_cls, types.FunctionType):
            decorator = annotate_method(annotation, key=key, unique=unique, single=single)
        elif inspect.isclass(func_or_cls):
            decorator = annotate_class(annotation, key=key, unique=unique, single=single)
        else:
...
def annotate_class(annotation: typing.Any, *, key=None, unique=False, single=False) -> typing.Callable:
    def wrapper(cls: typing.Type) -> typing.Type:
        component = Component.get_by_cls(cls)
        if annotation is not None:
            component.annotations.add(annotation, key=key, unique=unique, single=single)
        return cls

    return wrapper


def annotate_method(annotation: typing.Any, *, key=None, unique=False, single=False) -> typing.Callable:
    def wrapper(func_or_method: typing.Union[types.FunctionType, ComponentMethod]) -> ComponentMethod:
        ...
        if annotation is not None:
            method.annotations.add(annotation, key=key, unique=unique, single=single)
        return method
    return wrapper
